...
final ValueNotifier<bool> showAssetsNotifier = ValueNotifier(false);

void showAssetsWindow(BuildContext context) {
  if (showAssetsNotifier.value) return;
  SharedPreferences.getInstance().then((prefs) => prefs.setBool(VisualEditorScreen.getPrefKey('showAssets'), true));
  showAssetsNotifier.value = true;
}

void hideAssetsWindow() {
  showAssetsNotifier.value = false;
  SharedPreferences.getInstance().then((prefs) => prefs.setBool(VisualEditorScreen.getPrefKey('showAssets'), false));
}
...
class AssetsWindow extends StatefulWidget {
  ...
  const AssetsWindow({super.key, required this.onClose, this.onFocus, required this.onOpenTimeline, this.isDocked = false});

  @override
  State<AssetsWindow> createState() => _AssetsWindowState();
}
class _AssetsWindowState extends State<AssetsWindow> {
  ...
  Future<void> _loadPreferences() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _width = prefs.getDouble(VisualEditorScreen.getPrefKey('assets_w')) ?? 1000;
        _height = prefs.getDouble(VisualEditorScreen.getPrefKey('assets_h')) ?? 650;
        _bgOpacity = prefs.getDouble('ve_toolWindowOpacity') ?? 0.95;
        _isCollapsed = prefs.getBool(VisualEditorScreen.getPrefKey('assets_isCollapsed')) ?? false;
        double dx = prefs.getDouble(VisualEditorScreen.getPrefKey('assets_dx')) ?? 100;
        double dy = prefs.getDouble(VisualEditorScreen.getPrefKey('assets_dy')) ?? 100;
  ...
  Future<void> _savePreferences() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble(VisualEditorScreen.getPrefKey('assets_w'), _width);
    await prefs.setDouble(VisualEditorScreen.getPrefKey('assets_h'), _height);
    await prefs.setBool(VisualEditorScreen.getPrefKey('assets_isCollapsed'), _isCollapsed);
    await prefs.setDouble(VisualEditorScreen.getPrefKey('assets_dx'), _offset.dx);
    await prefs.setDouble(VisualEditorScreen.getPrefKey('assets_dy'), _offset.dy);
  }
  @override
  Widget build(BuildContext context) {
    if (!_isLoaded) return const SizedBox.shrink();

    if (widget.isDocked) return Material(color: Colors.transparent, child: AssetsPanel(onOpenTimeline: widget.onOpenTimeline));
    Widget rz({
                ...
                Listener(
                  ...
                  child: Container(
                    ...
                    clipBehavior: Clip.antiAlias, decoration: BoxDecoration(
                      border: AppUIConfig.windowBorderWidth > 0 ? Border.all(color: VisualEditorScreen.activeWindowNotifier.value == 'assets' ? AppColors.activeWindowBorder : AppColors.border, width: AppUIConfig.windowBorderWidth) : null,
                    ...
                    child: Column(children: [
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              Icon(AppToolWindows.getDef('assets').icon, size: 16, color: AppToolWindows.getDef('assets').color),
                              const SizedBox(width: 8),
                              Text(AppUIConfig.formatWindowTitle(AppToolWindows.getDef('assets').name), style: TextStyle(
                                      color: AppColors.titleBarTextPrimary,
                                      fontSize: AppUIConfig.windowTitleFontSize,
                                      fontWeight: AppUIConfig.windowTitleFontWeight)),
                                const Spacer(),
                                IconButton(
                                  icon: Icon(_isCollapsed ? Icons.expand_more : Icons.expand_less, size: 18, color: AppToolWindows.getDef('assets').color),
                                  onPressed: () {
                        ...
                        if (!_isCollapsed)
                          Expanded(
                            child: ClipRRect(
                              ...
                              child: AssetsPanel(onOpenTimeline: widget.onOpenTimeline),
                            ),
                          )
                      ]),
                    ),
