...
final ValueNotifier<bool> showLocalizationNotifier = ValueNotifier(false);

void showLocalizationWindow(BuildContext context) {
  if (showLocalizationNotifier.value) return;
  SharedPreferences.getInstance().then((prefs) => prefs.setBool(VisualEditorScreen.getPrefKey('showLocalization'), true));
  showLocalizationNotifier.value = true;
}

void hideLocalizationWindow() {
  showLocalizationNotifier.value = false;
  SharedPreferences.getInstance().then((prefs) => prefs.setBool(VisualEditorScreen.getPrefKey('showLocalization'), false));
}

class LocalizationWindow extends StatefulWidget {
  final bool isDocked;
  final VoidCallback onClose;
  final VoidCallback? onFocus;
  const LocalizationWindow({super.key, required this.onClose, this.onFocus, this.isDocked = false});

  @override
  State<LocalizationWindow> createState() => _LocalizationWindowState();
}
class _LocalizationWindowState extends State<LocalizationWindow> {
  ...
  Future<void> _loadPreferences() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _width = prefs.getDouble(VisualEditorScreen.getPrefKey('locale_w')) ?? 1000;
        _height = prefs.getDouble(VisualEditorScreen.getPrefKey('locale_h')) ?? 650;
        _bgOpacity = prefs.getDouble('ve_toolWindowOpacity') ?? 0.95;
        _isCollapsed = prefs.getBool(VisualEditorScreen.getPrefKey('locale_isCollapsed')) ?? false;
        double dx = prefs.getDouble(VisualEditorScreen.getPrefKey('locale_dx')) ?? 150;
        double dy = prefs.getDouble(VisualEditorScreen.getPrefKey('locale_dy')) ?? 150;
  ...
  Future<void> _savePreferences() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble(VisualEditorScreen.getPrefKey('locale_w'), _width);
    await prefs.setDouble(VisualEditorScreen.getPrefKey('locale_h'), _height);
    await prefs.setBool(VisualEditorScreen.getPrefKey('locale_isCollapsed'), _isCollapsed);
    await prefs.setDouble(VisualEditorScreen.getPrefKey('locale_dx'), _offset.dx);
    await prefs.setDouble(VisualEditorScreen.getPrefKey('locale_dy'), _offset.dy);
  }
  @override
  Widget build(BuildContext context) {
    if (!_isLoaded) return const SizedBox.shrink();

    if (widget.isDocked) return const Material(color: Colors.transparent, child: LocalizationPanel());
    Widget rz({
                ...
                Listener(
                  ...
                  child: Container(
                    ...
                    clipBehavior: Clip.antiAlias, decoration: BoxDecoration(
                      border: AppUIConfig.windowBorderWidth > 0 ? Border.all(color: VisualEditorScreen.activeWindowNotifier.value == 'localization' ? AppColors.activeWindowBorder : AppColors.border, width: AppUIConfig.windowBorderWidth) : null,
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(AppUIConfig.formatWindowTitle('Strings Definitions'), style: TextStyle(
