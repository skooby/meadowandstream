...
showColorPickerWindow(context);
  ...
  Future<void> _executeSingleTaskPrompt(AiTask task, [int mode = 0, bool silent = false, bool copyOnly = false]) async {
    ...
    bool bypassPreview = false;
    if (task.previewItems.isNotEmpty && task.previewItems.every((item) => item.isApproved)) {
        bypassPreview = true;
    }

    if (AiBridgeService.instance.isPreviewMode && !bypassPreview) {
      if (!copyOnly) {
        task.previewItems = [];
        AiBridgeService.instance.notifyListeners();
      }
      ...
      if (!copyOnly) {
        ...
        // preventing subsequent subtasks from inheriting them in the queue natively.
        task.previewItems = [];
        AiBridgeService.instance.saveTasks();
        AiBridgeService.instance.notifyListeners();
      }
                                        ...
                                        Builder(
                                            builder: (ctx) {
                                              ...
                                              return Padding(
                                                ...
                                                child: pendingFeedbackTask != null
                                                        ? IconButton(
                                                            onPressed: () => _showPreviewDialog(context, pendingFeedbackTask!),
                                                            icon: const _FlashingIcon(icon: Icons.rate_review, color: Colors.white, size: 20),
                                                            tooltip: 'Review Feedback',
                                                            constraints: const BoxConstraints(),
                                                            padding: const EdgeInsets.all(4),
                                                          )
                                                        : IconButton(
                                                            onPressed: null,
                                                            icon: Icon(Icons.rate_review, size: 20, color: Colors.white.withOpacity(0.5)),
                                                            tooltip: 'Review Feedback',
                                                            constraints: const BoxConstraints(),
                                                            padding: const EdgeInsets.all(4),
                                                          ),
  ...
  void _showPreviewDialog(BuildContext context, AiTask task) {
    showDialog(
      context: context,
      builder: (ctx) {
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                      ...
                      children: [
                        Text('Review Preview', style: TextStyle(color: AppColors.panelTextPrimary, fontSize: AppUIConfig.rootFontSize, fontWeight: FontWeight.bold)),
                        TextButton.icon(
                          onPressed: () {
                            AiBridgeService.instance.pushUndoState();
                            setStateBuilder(() {
                              for (var item in task.previewItems) {
                          ...
                          label: const Text('Accept All', style: TextStyle(color: Colors.greenAccent)),
                        ),
                      ],
                    ),
                  const SizedBox(height: 16),
                  Flexible(
                    child: Builder(
                      builder: (context) {
                        Map<String, List<AiPreviewItem>> groupedItems = {};
                        for (var item in task.previewItems) {
                          final cat = item.category?.toUpperCase() ?? 'UNGROUPED';
                          groupedItems.putIfAbsent(cat, () => []).add(item);
                        }

                        List<Widget> contentWidgets = [];
                        groupedItems.forEach((cat, items) {
                          Color headerColor = AppColors.titleBarTextSecondary;
                          if (cat == 'RISKY') headerColor = Colors.redAccent;
                          else if (cat == 'FEEDBACK') headerColor = Colors.orangeAccent;
                          else if (cat == 'SAFE') headerColor = Colors.greenAccent;

                          if (cat != 'UNGROUPED') {
                            contentWidgets.add(Padding(
                              padding: const EdgeInsets.only(top: 8, bottom: 8, left: 4),
                              child: Text(cat, style: TextStyle(color: headerColor, fontSize: AppUIConfig.smallFontSize, fontWeight: FontWeight.bold)),
                            ));
                          }

                          for (var item in items) {
                            final i = task.previewItems.indexOf(item);
                            contentWidgets.add(Container(
                              margin: const EdgeInsets.only(bottom: 12),
                              padding: const EdgeInsets.all(8),
                              decoration: BoxDecoration(color: AppColors.overlaySubtle, borderRadius: BorderRadius.circular(4)),
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.stretch,
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      Checkbox(
                                        ...
                                        onChanged: (val) {
                                          AiBridgeService.instance.pushUndoState();
                                          setStateBuilder(() => item.isApproved = val ?? false);
                                          AiBridgeService.instance.saveTasks();
                                        },
                                        fillColor: WidgetStateProperty.resolveWith((states) => states.contains(WidgetState.selected) ? Colors.lightBlueAccent
                                    : AppColors.titleBarTextSecondary),
                                      ),
                                      Expanded(child: Padding(padding: const EdgeInsets.only(top: 10), child: Text(item.description, style: TextStyle(color: Colors.white, fontSize: AppUIConfig.rootFontSize, fontWeight: FontWeight.w500)))),
                                      IconButton(
                                        icon: Icon(Icons.close, size: 16, color: AppColors.textMuted),
                                        onPressed: () {
                                          AiBridgeService.instance.pushUndoState();
                                          setStateBuilder(() {
                                            task.previewItems.removeAt(i);
                                            if (task.previewItems.isEmpty) {
                                              Navigator.pop(ctx);
                                            }
                                          });
                                          AiBridgeService.instance.saveTasks();
                                          GlobalPickerState.instance.requestIcon(
                                            initialIcon: IconData(task.iconCodePoint ?? 0xe2bc, fontFamily: 'MaterialIcons'),
                                            onIconSelected: (icon) {
                                              if (icon != null) {
                                                task.iconCodePoint = icon.codePoint;
                                                AiBridgeService.instance.saveTasks();
                                                AiBridgeService.instance.notifyListeners();
                                              }
                                            }
                                  ...
                                  Padding(
                                    ...
                                    child: TextFormField(
                                      ...
                                      style: TextStyle(color: AppColors.panelTextPrimary, fontSize: AppUIConfig.rootFontSize),
                                      decoration: InputDecoration(
                                        hintText: 'Add a comment / correction for the AI...',
                  ...
                  Row(
                    ...
                    children: [
                      TextButton(onPressed: () async { 
                        ...
                        AiBridgeService.instance.notifyListeners(); 
                        Navigator.pop(ctx); 
                      }, child: Text('Save & Close', style: TextStyle(color: AppColors.panelTextSecondary))),
                      const SizedBox(width: 8),
                      ElevatedButton(
                        ...
                        onPressed: () async {
                            ...
                            final hasTasksToPerform = task.verificationCriteria.any((e) => (e.status != AiVerificationStatus.verified && e.status != AiVerificationStatus.ignored));
                            if (hasTasksToPerform) {
                                task.status = AiTaskStatus.inProgress;
                            }
                            AiBridgeService.instance.saveTasks();
                            bool allApproved = task.previewItems.isNotEmpty && task.previewItems.every((item) => item.isApproved);
                            if (allApproved) {
                              Navigator.pop(ctx);
                              await _executeSingleTaskPrompt(task, 0, true);
                              AiBridgeService.instance.forceNextQueueItem();
                              if (mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Task approved and natively queued for execution!')));
                              }
                              return;
                            } else {
                              final rejectedItems = task.previewItems.where((i) => !i.isApproved).toList();
                              final StringBuffer sb = StringBuffer();
                              sb.writeln('Here is my review of the rejected preview items you generated:');
                              for (var item in rejectedItems) {
                                sb.writeln('- ${item.description}');
                                sb.writeln('  Approved: NO');
                                if (item.comment.isNotEmpty) {
                                  sb.writeln('  Comment: ${item.comment}');
                                }
                              }
                              sb.writeln('\\n\${AiBridgeService.instance.previewRejectedInstructions}');
                              await AiBridgeService.instance.sendToQueue(sb.toString(), true, taskIds: [task.id], insertFirst: true);
                              AiBridgeService.instance.forceNextQueueItem();
                              
                              task.previewItems.removeWhere((i) => !i.isApproved);
                              AiBridgeService.instance.saveTasks();
                              AiBridgeService.instance.notifyListeners();
  
                              Navigator.pop(ctx);
                              if (mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Sent preview feedback to queue!')));
                              }
                            }
                          },
                        child: Text('Send', style: TextStyle(color: AppColors.panelTextPrimary)),
                      ),
                    ],
                  ),
                ],
