...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    ...
    // Workaround: disable subtitle auto-loading to avoid fatal mpv errors 
    // when local .lrc files exist next to the media file on Windows.
    setProperty(_player, 'sub-auto', 'no');

    if (Platform.isWindows) {
      // Workaround: prevent libmpv c-layer exception and complete native release 
      // if no audio devices are physically connected (Servers, RDP, unplugged speakers).
      setProperty(_player, 'ao', 'wasapi,openal,null');
    }
    ...
    _streamSubscriptions = [
      ...
      _player.stream.error.listen((error) {
        // Suppress non-fatal subtitle and mpv property errors
        if (error.contains('Can not open external file') || error.contains('_setProperty')) {
            _logger.warning('Ignored mpv external/property error: $error');
            return;
        }
        ...
        if (errorUri == null || errorUri == _currentMedia?.uri) {
          ...
          _updatePlaybackEvent();
          if (_loadCompleter?.isCompleted != true) {
            _loadCompleter?.completeError(PlatformException(code: _errorCode.toString(), message: _errorMessage));
          }
        }
    ...
    switch (audioSource) {
      case final UriAudioSourceMessage uriSource:
        ...
        if (uri.startsWith('file://')) {
          try {
            uri = Uri.parse(uri).toFilePath().replaceAll('\\', '/');
          } catch (_) {}
        }
        return Media(uri, httpHeaders: audioSource.headers);
